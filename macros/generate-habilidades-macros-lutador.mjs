...
(async () => {
  ...
  // Ícones inteligentes para Lutador
  const ICON_MAP = [
  ...
  function formatDescription(text) {
    ...
    // Quebra dupla = parágrafo, quebra simples = br
    return text.split(/\n\s*\n/).map(p => `<p>${p.trim().replace(/\n/g, "<br>")}</p>`).join("");
  }

  function slugToCamelKey(name = "") {
  ...
  for (const entry of APTIDOES) {
    ...
    const key = slugToCamelKey(entry.name);
    const aptidaoKey = `lutador.${key}`;
    ...
    const itemData = {
      ...
      system: {
        ...
        // Metadados para validação automática na ficha:
        // - `classe`: nome da classe que destrava essa aptidão (ex.: "Lutador")
        // - `nivelMin`: nível mínimo na classe
        // Esses campos não estão no template.json padrão, mas o Foundry aceita `system.*` extra.
        classe: { value: catName, label: "Classe" },
