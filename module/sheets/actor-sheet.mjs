...
/** @override */
  ...
  async _onDropItemCreate(itemData, event) {
    try {
      ...
      return await super._onDropItemCreate(itemData, event);
    } finally {
  ...
  async _rollPericiaByKey({ key, group = 'pericias' } = {}) {
    ...
    await roll.toMessage({
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
        flavor: `Rolagem de <b>${pericia.label}</b>`
    });
      ...
      if (isMeleeAttack) {
        ...
        if (d20Face === 20) {
          ...
          try {
            ...
            await rollD6.evaluate({async: false});
            ...
            if (next) {
              ...
              if (Array.isArray(prereq?.prereqTokens) && prereq.prereqTokens.length > 0) {
                for (const token of prereq.prereqTokens) {
                  ...
                  if (!resolved) {
                    // Se não conseguimos mapear, não bloqueia (evita falsos positivos), mas registra.
                    console.warn('Pré-requisito não mapeado no catálogo:', token, 'para', `${cat}.${key}`);
                    continue;
                  }
...
/**
  ...
  async _onLevelUp(event) {
    ...
    // --- CENÁRIO 1: SEM MULTICLASSE ---
    if (!temMulticlasse) {
        await this.actor.update({ "system.detalhes.niveis.principal.value": nivelPrincipal + 1 });
        ui.notifications.info(`${classePrincipalNome} subiu de nível!`);
        return;
    }

// --- CENÁRIO 2: COM MULTICLASSE (CSS PURO) ---
      new Dialog({
        title: "Ascensão de Grau",
        content: `
          <div class="level-up-header">
            <h2>Nível Disponível!</h2>
            <p>Escolha o caminho para evoluir sua técnica.</p>
          </div>
        `,
        buttons: {
          principal: {
            label: `
            <i class="fas fa-fist-raised level-opt-icon"></i>
            <div class="level-opt-info">
              <span class="level-opt-title">${classePrincipalNome}</span>
              <span class="level-opt-desc">
                Nível ${nivelPrincipal} <span class="arrow-up">➜</span> ${nivelPrincipal + 1}
              </span>
            </div>`,
            callback: async () => {
              await this.actor.update({"system.detalhes.niveis.principal.value": nivelPrincipal + 1});
              ui.notifications.info(`Subiu nível de ${classePrincipalNome}!`);
            }
          },
          secundaria: {
            label: `
            <i class="fas fa-magic level-opt-icon"></i>
            <div class="level-opt-info">
              <span class="level-opt-title">${multiclasseNome}</span>
              <span class="level-opt-desc">
                Nível ${nivelSecundario} <span class="arrow-up">➜</span> ${nivelSecundario + 1}
              </span>
            </div>`,
            callback: async () => {
              await this.actor.update({"system.detalhes.niveis.secundario.value": nivelSecundario + 1});
              ui.notifications.info(`Subiu nível de ${multiclasseNome}!`);
            }
          }
        },
        default: "principal"
